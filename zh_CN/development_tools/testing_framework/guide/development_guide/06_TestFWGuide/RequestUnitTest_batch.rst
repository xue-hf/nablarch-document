...
* テスト用のコンポーネント設定ファイルからSystem Repositoryを初期化する。
* 常駐化機能を無効化する。
...
驻留型Batchのテスト用ハンドラ構成
-----------------------------------------------------
驻留型Batchのテストを実施する際には、プロダクション用ハンドラ構成をテスト用に変更する必要がある。
この変更をせずにテストを実施した場合、テスト対象の驻留型Batch应用の処理が終わらないため、テストが正常に実施できなくなる。
