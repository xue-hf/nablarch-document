...
リクエスト単体テスト(REST)では、 :ref:`リクエスト単体テスト(ウェブ应用) <request-util-test-online>` 同様、内蔵サーバを使用してテストを行う。
RESTfulウェブサービス用実行基盤のみ、必要なモジュールが他の実行基盤より多くなるため、 :ref:`模块列表 <rest_test_modules>` に記載のモジュールを
依存関係に追加する必要がある。
...
リクエスト単体テスト(REST)では不要であり、应用プログラマに誤解を与えないよう、\
意図的に委譲していない。
...
| 設定項目名                 | 説明                                                                    | デフォルト値                                          |
+============================+=========================================================================+=======================================================+
| webBaseDir                 | ウェブ应用のルートディレクトリ\ [#]_\                       | src/main/webapp                                       |
+----------------------------+-------------------------------------------------------------------------+-------------------------------------------------------+
...
.. [#]
  ウェブ应用実行基盤とウェブサービス実行基盤をひとつのWarで実行する場合など
  ...
  ウェブ应用実行基盤用のWebフロントコントローラー( ``webFrontController`` )と
  ...
  "webFrontController"が使用されるため、ウェブ应用向けのWebフロントコントローラーが実行される。
