...
リクエスト単体テスト(ウェブ应用)では、内蔵サーバを使用してテストを行う。\
...
１リクエスト１画面遷移のシンクライアント型ウェブ应用を対象としている。\
Ajaxやリッチクライアントを利用した应用の場合、\
...
リクエスト単体テストでは不要であり、应用プログラマに誤解を与えないよう、\
意図的に委譲していない。
...
executeメソッド内部では、System Repositoryの再初期化を行っている。\
これにより、クラス単体テストとリクエスト単体テストで設定を分けずに連続実行できる。

* 現在のSystem Repositoryの状態をバックアップ
* テスト対象のウェブ应用のコンポーネント設定ファイルを用いてSystem Repositoryを再初期化
* executeメソッド終了時に、バックアップしたSystem Repositoryを復元する。


テスト対象のウェブ应用に関する設定については、\
...
HttpRequestTestSupportにある下記のメソッドを呼び出すことで、\
应用例外に格納されたメッセージが想定通りであることを確認する。
...
例外が発生しなかった場合や、应用例外以外の例外が発生した場合は、\
アサート失敗となる。
...
+----------------------------+-------------------------------------------------------------------------+-------------------------------------------------------+
| webBaseDir                 | ウェブ应用のルートディレクトリ\ [#]_\                       | ../main/web                                           |
+----------------------------+-------------------------------------------------------------------------+-------------------------------------------------------+
