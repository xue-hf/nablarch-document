...
本アダプタを使用することで、 :ref:`ウェブ应用 <web_application>` や :ref:`RESTfulウェブサービス <restful_web_service>` を
...
ディスパッチハンドラを設定する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
ディスパッチハンドラとして、 :java:extdoc:`RoutesMapping <nablarch.integration.router.RoutesMapping>` をhandler队列の最後に設定する。

設定例を以下に示す。

ポイント
 * コンポーネント名は **packageMapping** とする。
 * basePackage属性には、Action类が格納されているパッケージを設定する。
   (Action类が複数のパッケージに格納されている場合は、共通となる親パッケージを設定する。)
...
.. important::
  ...
  应用サーバに `JBoss` や `WildFly` を使用している場合、この機能は使用できない。
...
.. important::
  ...
  そのようなウェブ应用サーバを使用する場合は、従来通りXMLを用いたルーティングの定義を使用すること。
...
| Action类を ``Path`` アノテーションで注釈することで、 ``Path`` アノテーションの ``value`` で設定したパスとAction类を紐づけることができる。
| さらに、 ``jakarta.ws.rs.GET`` などのHTTPメソッドを表すアノテーションでAction类のメソッドを注釈することで、HTTPメソッドとAction类のメソッドを紐づけることができる。
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
Action类は、実装しているインターフェースや継承している親クラスに注釈されている ``Path`` アノテーションやメソッドに注釈されているアノテーションの内容を引き継ぐことができる。
...
.. tip::
  ...
  Action类が他のクラスを継承していたりインターフェースを実装している場合はその階層をたどり、最初に見つかった ``Path`` アノテーションが型定義に注釈されているクラスまたはインターフェースに宣言されているメソッドをリクエストを受け付けるメソッドとして認識する動作となる。
