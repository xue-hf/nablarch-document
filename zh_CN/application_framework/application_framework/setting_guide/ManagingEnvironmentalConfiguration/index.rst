...
そのため、应用設定もそれぞれの処理方式、環境に合わせて複数管理する必要がある。

本章节、Nablarchにおける処理方式、環境に依存する設定の管理方法を示す。
...
Nablarchでは、应用設定を２つの観点で整理することを推奨している。
...
应用設定ファイルは、処理方式と環境の組み合わせで最小限必要になるものを用意する。

以下に、アーキタイプから生成した直後のプロジェクトの应用設定ファイルに関連するディレクトリ構造を示す。
 ...
 * 実行基盤のプロジェクト(ウェブ应用、バッチ应用等)から参照される共通プロジェクトを使用している場合、共通プロジェクト単体の環境毎の应用設定ファイルは不要である。
...
類似しているプロファイルの应用設定ファイルをコピーし、修正する。
