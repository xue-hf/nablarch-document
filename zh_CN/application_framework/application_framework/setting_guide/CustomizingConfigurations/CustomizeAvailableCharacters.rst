...
アーキタイプから生成したプロジェクトで、使用可能文字の精査エラーメッセージの設定方法は以下の3パターンがある。

* メッセージIDを設定するだけで使用できるもの
* メッセージIDを設定しただけでは使用可能にならないコンポーネント定義が必要なもの
* 単独では使用できないもの
...
使用可能な文字集合は、複数の文字集合から構成される(即ち包含関係が存在する)。
...
指定するメッセージIDに対応するプレースホルダは :download:`默认设置一览 <../../configuration/默认设置一览.xlsx>` を参照
...
.. tip::
  ...
  メッセージIDを定義していない場合、Nablarch应用起動時に警告が出力されてしまうためである。
