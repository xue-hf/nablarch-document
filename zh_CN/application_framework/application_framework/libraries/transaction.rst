...
.. tip::
  ...
  バッチ应用などの大量データを一括で処理するような機能では、トランザクションタイムアウト機能を使用するのではなく、
  ジョブスケジューラの終了遅延監視などで処理遅延のハンドリングを行うこと。

  なぜなら、バッチ应用では、全体の処理時間が想定内であればよく、個々のトランザクションで遅延が起きても問題ないからである。
...
トランザクションタイムアウトのチェックタイミング
  ...
  .. tip::
    ...
    このような場合には、应用サーバのタイムアウト機能などを用いて、遅延している应用スレッドのハンドリングを行うこと。
