...
初期セットアップで作成するブランクプロジェクトは、初期構築の容易さを重視している。そのため、1プロジェクトで各应用をビルドできるように、全てのソースファイルとリソースファイルを1プロジェクトに配置している。
...
* システムを構成する应用が複数(ウェブ应用とバッチ应用等)存在する。
* 应用間で共通の部品(例えば、Entityクラス)が存在する。
...
.. list-table::
  ...
  * - ソフトウェア
    - 説明
  * - APサーバ
    - ウェブプロジェクト及び RESTfulウェブサービスプロジェクトの疎通確認時にJetty12を使用する。手順中で、mvnコマンドからjetty-ee10-maven-pluginを実行し、jetty-ee10-maven-pluginに組み込まれているJetty12へ应用をデプロイ、起動するため、事前準備は不要である。
  * - DBサーバ
