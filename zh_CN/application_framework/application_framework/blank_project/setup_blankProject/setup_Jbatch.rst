...
.. list-table::
  ...
  * - プロジェクト構成
    - 単一プロジェクト構成
  * - 使用数据库
    - H2 Database Engine(应用に組み込み)
  * - 生成するプロジェクトに含まれるもの
    - 生成されたプロジェクトには以下が含まれる。
       
      * Jakarta Batchに準拠したバッチ应用用の基本的な設定
      * batchlet方式による疎通確認用バッチ应用
      * chunk方式による疎通確認用バッチ应用
...
生成したプロジェクトには、以下のバッチ应用が組み込まれている。

=================== ================================================================================
ジョブID            内容
=================== ================================================================================
sample-chunk        chunk方式で実装されたサンプル应用。
sample-batchlet     batchlet方式で実装されたサンプル应用。
=================== ================================================================================


上記2つのバッチ应用の動作確認を行うことで、ブランクプロジェクトの生成に成功していることを確認する。
...
chunk方式のバッチ应用の起動
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
chunk方式のバッチ应用では、SAMPLE_USERテーブルからデータを取り出し、編集してCSVファイルに出力する処理が実装されている。
...
batchlet方式のバッチ应用の起動
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
batchlet方式のバッチ应用では、SAMPLE_USERテーブルのデータを削除する処理が実装されている。
