...
.. list-table::
  ...
  * - nablarch-web-archetype
    - ウェブ应用実行制御基盤を使用する場合のアーキタイプ
  * - nablarch-jaxrs-archetype
    - RESTfulウェブサービス実行制御基盤を使用する場合のアーキタイプ
  * - nablarch-batch-ee-archetype
    - Jakarta Batchに準拠したバッチ应用フレームワークを使用する場合のアーキタイプ
  * - nablarch-batch-archetype
    - Nablarch独自のバッチ应用実行制御基盤を使用する場合のアーキタイプ
  * - nablarch-batch-dbless-archetype
    - Nablarch独自のバッチ应用実行制御基盤を使用するがDBに接続しない場合のアーキタイプ
...
.. list-table::
  ...
  * - pj-web
    - war
    - ウェブ应用実行制御基盤を使用する应用を開発する。
      
      最終的に、warファイルとして应用サーバにデプロイされる単位で作成する。
  * - pj-batch 
    - jar
    - Nablarch独自のバッチ应用実行制御基盤を使用する应用を開発する。
...
ウェブ应用のwarファイルとしてパッケージングされるプロジェクト。
...
.. code-block:: text
    ...
    |
    +---db                          … 疎通应用用のDDL及びInsert文。RDBMS別に格納されている。
    |
    +---h2
    ...
    |
    +---tmp                         … ウェブ应用のリクエスト単体テストで使用されるディレクトリ。疎通テスト実行時に自動生成される。
    |
...
RESTfulウェブサービス应用のwarファイルとしてパッケージングされるプロジェクト。
...
Jakarta Batchに準拠したバッチ应用のjarファイルとしてパッケージされるプロジェクト。
...
.. code-block:: text
    ...
    |   |           |
    |   |           \---batch-jobs
    |   |                   sample-batchlet.xml … batchlet方式の疎通確認用应用のジョブファイル。
    |   |                   sample-chunk.xml    … chunk方式の疎通確認用应用のジョブファイル。
    |   |
    |   \---test
    |       +---java
    |       |
...
バッチ应用のビルド時に ``target`` 配下に生成されるzipファイルの中には、
バッチ应用の実行可能jarと依存ライブラリが格納されている。
...
ウェブ应用がデプロイされたTomcatベースのDockerイメージをビルドするプロジェクト。
...
RESTfulウェブサービス应用がデプロイされたTomcatベースのDockerイメージをビルドするプロジェクト。
...
* 作成する应用が１つの場合(ウェブのみ、バッチのみ等)は、それぞれ単体のプロジェクトで構成する。
* 社内用と社外用で２つのウェブ应用を作成するようなケースでは、無理に１つのMavenプロジェクトにまとめず、個別にMavenプロジェクトを作ること。
* 複数の应用が存在し、共通化したいライブラリが存在する場合は、共通ライブラリを配置するMavenプロジェクトを作る。
* 実行制御基盤を追加した際は、実行制御基盤毎にMavenプロジェクトを作る。例えば、メッセージング実行制御基盤を使用した应用を追加する場合は、新しくMavenプロジェクトを作る。
