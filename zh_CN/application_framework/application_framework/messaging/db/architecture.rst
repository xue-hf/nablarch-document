...
構成
--------------------------------------------------
Nablarch Batch应用と同じ構成となる。
詳細は、 :ref:`Nablarch Batch应用の構成 <nablarch_batch-structure>` を参照。


リクエストパスによるアクションとリクエストIDの指定
--------------------------------------------------
データベースをキューとして扱うメッセージング処理では、Nablarch Batch应用と同じように
コマンドライン引数で実行するアクションとリクエストIDを指定する。

詳細は、 :ref:`Nablarch Batch应用のアクションとリクエストIDの指定 <nablarch_batch-resolve_action>` を参照。

処理の流れ
------------------------------------------------------
Nablarch Batch应用と同じ処理の流れとなる。詳細は、 :ref:`nablarch_batch-process_flow` を参照。
...
ハンドラの最小構成
--------------------------------------------------
データベースをキューとして扱うメッセージング処理の必要最小限のhandler队列を以下に示す。
