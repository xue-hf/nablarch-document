...
この場合、ウェブ应用などのリクエストパスとは異なり、リクエストIDには階層構造が含まれないので、
:ref:`request_path_java_package_mapping` を使用し、Action类のパッケージやクラス名のサフィックスを設定で指定し、
...
1. :ref:`共通起動ランチャ(Main) <main>` がhandler队列(handler queue)を実行する。
2. データリーダ(
...
3. handler队列に設定された
   ...
   要求電文の特定フィールドに含まれるリクエストIDを元に処理すべきAction类(action class)を特定し、
   handler队列の末尾に追加する。
4. Action类(action class)は、フォームクラス(form class)やエンティティクラス(entity class)を使用して、
   要求電文1件ごとの業務ロジック(business logic) を実行する。
5. Action类(action class)は、応答電文を表す
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
同期応答メッセージングを構築する際の、必要最小限のhandler队列を以下に示す。
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
応答不要メッセージングを構築する際の、必要最小限のhandler队列を以下に示す。
これをベースに、プロジェクト要件に従ってNablarchの標準ハンドラやプロジェクトで作成したカスタムハンドラを追加する。
...
---------------------------------------------------------------------------------
Nablarchでは、MOMメッセージングを構築するために必要なAction类を標準で幾つか提供している。
...
* :java:extdoc:`AsyncMessageReceiveAction (応答不要メッセージングのAction类)<nablarch.fw.messaging.action.AsyncMessageReceiveAction>`
