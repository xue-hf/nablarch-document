...
本サンプルは、Nablarch应用フレームワークの :ref:`mom_system_messaging` 、 :ref:`http_system_messaging` を使用する
应用のテストにて、対向先システムをシミュレートするサンプル実装を提供する。

テスト環境構築後の疎通テストや应用の結合テスト等で、应用の接続先として、
使用することを想定している。
...
テスト環境を構築後のNablarch应用フレームワーク、ミドルウェア、ハードウェア等の設定を確認するための疎通テストにて、
簡易的に通信する対向先システムとして使用できる。

結合テスト
~~~~~~~~~~~

应用の結合テストの際に、システム間通信の擬似的な対向先システムとして使用できる。
...
应用の負荷テストを行う際に、負荷をかけるための大量のメッセージ送受信ができる。
...
Nablarch应用フレームワークのマルチスレッド実行制御ハンドラをシミュレータ内部で使用しており、
...
リクエストの種類に応じてレスポンスを切り替えるには、Action类の ``getRequestId`` メソッドを修正する。
...
.. tip::
  ...
  Action类の ``getRequestId`` メソッドを修正すればよい。

メッセージ受信時に意図的にレスポンスを遅延させる
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

メッセージ受信時に意図的にレスポンスを遅延させるには、
以下の様にAction类の ``handle`` メソッドに直接遅延処理を実装する。
