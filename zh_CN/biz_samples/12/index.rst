...
IDトークンの取得方法や実装場所などは应用構成によって様々あるが、本サンプルでは以下の構成を想定している。

* Nablarchを使用したRESTfulウェブサービスの应用を、バックエンドとして稼働させる
...
本サンプルで取り扱うのはNablarchを使用するバックエンド应用のみとし、フロントエンド应用（Web、モバイルアプリ等）で行うIDトークンの取得方法については本サンプルの対象外とする。
...
aws.cognito.userPool.clientId   ユーザープールに登録した应用のクライアントID
=============================== =================================================================================================
...
.. code-block:: java
  ...
  private DecodedJWT verifyIdToken(String idToken) {
      // プロパティを使用した検証用コンポーネントを定義しているため、System Repositoryから取得する
